...
class Settings(BaseSettings):
    ...
    # Whether to request the sensitive data scopes at all. False = login only
    # (email/profile), no consent warning of any kind.
    google_data_scopes: bool = True
    # THE EDITION SWITCH. Gmail read is a RESTRICTED scope: verifying it needs a
    # paid CASA security assessment (months, thousands/yr). Calendar and Gmail
    # *send* are only "sensitive" — free verification, days.
    #   True  → Personal edition: Gmail + Calendar. For your own use.
    #   False → Public edition: Calendar only. Verifiable free, no scary screen.
    # Nothing else differs between the two builds. See docs/GOOGLE_VERIFICATION.md
    google_gmail_scopes: bool = True
